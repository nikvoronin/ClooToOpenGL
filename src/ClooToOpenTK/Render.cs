...
namespace ClooToOpenTK
{
    ...
    {
        public static ComputeDevice cDevice;
        ...
        public string ComputePlatformName => cDevice?.Platform.Name;

        public static Render CreateCloo(uint width, uint height, Render cloo = null, string vendor = null)
        {
            string kernelSource = File.ReadAllText(Const.KERNEL_FILENAME); //LoadEmbeddedFile(KERNEL_FILENAME);
            ...
            foreach (var p in ComputePlatform.Platforms)
                foreach (var d in p.Devices)
                    if (d.Vendor.ToUpperInvariant().Contains(vendor))
                    {
                        cDevice = d;
                        break;
                    }
            ...
            if (cloo != null)
            {
                cloo =
                    new Render(
                        ...
                        width, height,
                        width * height / 10,
                        cloo.reMin, cloo.reMax, cloo.imMin, cloo.imMax,
                        cloo.maxIter
                        );
            }
            else
